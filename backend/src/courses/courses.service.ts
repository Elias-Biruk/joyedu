...
export class CoursesService {
  ...
  async create(instructorId: string, dto: CreateCourseDto) {
    ...
    return this.prisma.course.create({
      ...
      include: { category: true, instructor: { select: USER_BRIEF_SELECT } },
    });
  }
  ...
  async findAll(filters: CourseFilterDto) {
    ...
    if (filters.search) {
      where.OR = [
        ...textSearch(["title", "description"], filters.search),
        { tags: { hasSome: [filters.search] } },
      ];
    }
    if (filters.categoryId) where.categoryId = filters.categoryId;
    if (filters.difficulty)
      where.difficulty = filters.difficulty as Prisma.EnumDifficultyLevelFilter;
  ...
  async findBySlug(slug: string) {
    const course = await this.prisma.course.findUnique({
      ...
      include: {
        category: true,
        instructor: {
          select: { ...USER_BRIEF_SELECT, bio: true },
        },
        chapters: {
  ...
  async findById(id: string, userId: string, userRoles: string[]) {
    const course = await findCourseOrThrow(this.prisma, id);
  ...
  async update(id: string, instructorId: string, dto: UpdateCourseDto) {
    await findCourseAsOwner(this.prisma, id, instructorId);
